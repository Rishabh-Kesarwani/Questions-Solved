...
public class SortKsortedDLL {
	...
	// function to sort a k sorted doubly linked list Using Insertion Sort Time Complexity: O(n*k)
	...
	Node sortAKSortedDLL( Node head, int k) {
	    ...
	    for(Node i = head.next; i != null; i = i.next) {
	        Node j = i;
// There will be atmost k swaps for each element in the list since each node is k steps away
//	        from its correct position
	 ...
	 class compareNode implements Comparator<Node>
	  ...
	  Node sortAKSortedDLLbetter(Node head, int k) {
		 if(head==null || head.next==null) {
			 return head;
		 }
...
/* Function to print nodes in a given doubly linked list This function is same as printList()
  of singly linked list */
	...
	{
	    SortKsortedDLL list = new SortKsortedDLL();
 /* Let us create a k sorted doubly linked list to test the functions Created doubly linked list
    will be 3<->6<->2<->12<->56<->8 */
	    ...
	    Node sortedDLL = list.sortAKSortedDLLbetter(head, k);
	    System.out.println("");
